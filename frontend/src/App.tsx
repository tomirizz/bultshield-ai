...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><p>BultShield · 0.9.0</p><p>Статический анализ кода</p></div>
    </aside>
    <div className="app-content">
      ...
      <main className="main-content">
        ...
        {view === 'system' && <>
          ...
          <section className="panel system-details"><div className="panel-heading"><h2>Сведения о приложении</h2></div><dl className="system-facts"><div><dt>Версия</dt><dd>0.9.0</dd></div><div><dt>База данных</dt><dd>{readiness?.database || 'Недоступна'}</dd></div><div><dt>Версия схемы</dt><dd>{readiness?.schema_revision || '—'}</dd></div><div><dt>Окружение</dt><dd>{readiness?.environment === 'production' ? 'Рабочее' : readiness?.environment === 'development' ? 'Локальное' : '—'}</dd></div></dl><a className="text-button" href="/api/openapi.json" target="_blank" rel="noreferrer">Описание API<ExternalLink size={13} /></a></section>
        </>}
        <footer className="footer"><span><ShieldCheck size={14} />BultShield</span><span>Версия 0.9.0</span></footer>
      </main>
    </div>

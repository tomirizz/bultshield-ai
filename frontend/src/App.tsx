...
function ProjectForm({ onClose, onSaved }: { onClose: () => void; onSaved: (project: Project) => void }) {
  ...
  return <Dialog title="Новый проект" onClose={busy ? () => {} : onClose}><form onSubmit={submit}>
    ...
    <p className="form-help">После создания проекта нажмите «Запустить Gitleaks». Доступность репозитория проверяется при запуске.</p>
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="stage-progress"><span>Этап 03</span><span>Gitleaks</span></div><div className="stage-track"><span /></div><p>GitHub → Gitleaks → PostgreSQL → Dashboard</p><div className="hosting-label"><Server size={15} /> Целевая платформа: Bult.ai</div></div>
    </aside>
    <div className="app-content">
      ...
      <main className="main-content">
        <div className="page-heading"><div><div className="eyebrow">BULTSHIELD AI / SECRET SCANNING</div><h1>{view === 'workspace' ? 'Безопасность начинается здесь.' : view === 'findings' ? 'Единый список находок' : view === 'activity' ? 'История сканирований' : 'Основа системы'}</h1><p>{view === 'workspace' ? 'Добавьте публичный GitHub-репозиторий и проверьте его на секреты.' : view === 'findings' ? 'Находки Gitleaks. Значения секретов скрыты; действительность ключей не проверяется.' : view === 'activity' ? 'Каждый запуск, его статус и результат — в одном месте.' : 'Целевая платформа — Bult.ai. Архитектура готова к расширению.'}</p></div><div className="heading-actions"><button className="button secondary compact" onClick={() => void refresh()} disabled={loading} aria-label="Обновить данные"><RefreshCw size={16} className={loading ? 'spin' : ''} /><span>Обновить</span></button>{view === 'workspace' && <button className="button primary" onClick={() => setCreateOpen(true)} disabled={!isReady}><Plus size={16} />Новый проект</button>}</div></div>
        ...
        {view === 'workspace' && <>
          ...
          <div className="bottom-grid"><section className="panel scanner-panel"><div className="panel-heading"><div><h2>Security toolkit</h2><p>Четыре инструмента · один формат findings</p></div><span className="tag neutral">Gitleaks v8.30.1</span></div><div className="scanner-list">{tools.map(tool => <div className="scanner-row" key={tool.name}><div className="tool-icon"><tool.icon size={18} /></div><div><strong>{tool.name}</strong><small>{tool.role}</small></div><span className="tool-status"><Circle size={7} />{tool.name === 'Gitleaks' ? 'Подключён' : 'Следующий этап'}</span></div>)}</div></section>
        ...
        {view === 'findings' && <section className="panel"><div className="panel-heading"><div><h2>Findings <span className="count-badge">{overview?.findings ?? '—'}</span></h2><p>Gitleaks · максимум 100 последних находок · HIGH — оценка MVP</p><label className="filter-label">Сканирование<select value={findingScanId} onChange={event => { setFindings([]); setFindingScanId(event.target.value); }}><option value="">Все последние</option>{scans.map(scan => <option key={scan.id} value={scan.id}>{date(scan.created_at)} · {scan.status} · {scan.id.slice(0, 8)}</option>)}</select></label></div><label className="filter-label">Severity<select value={severity} onChange={event => setSeverity(event.target.value)}><option value="ALL">Все уровни</option>{['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO', 'UNKNOWN'].map(value => <option key={value}>{value}</option>)}</select></label></div>{filteredFindings.length ? <div className="result-list">{filteredFindings.map(finding => <article key={finding.id}><span className={`severity severity-${finding.severity.toLowerCase()}`}>{finding.severity}</span><div><h3>{finding.title}</h3><p>{finding.scanner} · {finding.file || finding.category}{finding.line_start ? `:${finding.line_start}` : ''}</p></div><span className="tag neutral">{finding.status}</span><details><summary>Детали</summary><p>{finding.description}</p><p>Правило: {finding.rule_id}</p><p>Секрет: <code>{finding.evidence}</code></p><p>Scan: {finding.scan_id.slice(0, 8)}</p></details></article>)}</div> : <EmptyState icon={<ShieldEllipsis size={32} />} title={findings.length ? 'Нет находок с таким уровнем' : 'Нет находок в выбранных результатах'}>Проверьте статус во вкладке «Сканирования». Ноль находок при COMPLETED означает, что правила Gitleaks ничего не обнаружили в текущих файлах выбранной ветки.</EmptyState>}</section>}
        {view === 'activity' && <section className="panel"><div className="panel-heading"><div><h2>Запуски <span className="count-badge">{overview?.scans ?? '—'}</span></h2><p>История сохраняется в PostgreSQL</p></div><span className="tag neutral">Gitleaks · обновление каждые 5 сек</span></div>{scans.length ? <div className="result-list">{scans.map(scan => <article key={scan.id}><ScanLine size={19} /><div><h3>{projects.find(project => project.id === scan.project_id)?.name || 'Project'}</h3><p>{date(scan.created_at)} · {scan.commit_sha?.slice(0, 7) || 'Ожидание commit SHA'}</p><p>{scan.scanner_results.gitleaks?.finding_count !== undefined ? `Найдено: ${scan.scanner_results.gitleaks.finding_count}` : 'Текущие файлы ветки · Gitleaks'}</p>{scan.error_message && <p className="form-error" role="alert">{scan.error_message}</p>}</div><span className={`tag ${scan.status === 'COMPLETED' ? 'mint' : 'neutral'}`}>{scan.status}</span>{scan.status === 'COMPLETED' && <button className="text-button" onClick={() => { setFindings([]); setFindingScanId(scan.id); setView('findings'); }}>Результаты <ArrowRight size={14} /></button>}</article>)}</div> : <EmptyState icon={<Activity size={31} />} title="Первый scan ещё впереди">Откройте проект и нажмите «Запустить Gitleaks» рядом с репозиторием.</EmptyState>}<div className="lifecycle-strip">{['QUEUED', 'CLONING', 'SCANNING', 'NORMALIZING', 'COMPLETED'].map((state, i) => <span key={state}>{i > 0 && <ArrowRight size={12} />}{state}</span>)}</div></section>}
        {view === 'system' && <>
          <section className="panel"><div className="panel-heading"><div><h2>Bult.ai infrastructure</h2><p>App, PostgreSQL и отдельный worker Gitleaks</p></div><span className="tag mint">Этап 03</span></div><div className="service-grid">{[{ name: 'App', desc: 'React + FastAPI', icon: LayoutDashboard, state: isReady ? 'Работает' : 'Проверяется', enabled: isReady }, { name: 'PostgreSQL', desc: 'Данные + схема очереди', icon: Database, state: isReady ? 'Подключена' : 'Нет соединения', enabled: isReady }, { name: 'Worker', desc: 'Gitleaks v8.30.1 · очередь PostgreSQL', icon: ScanLine, state: scans.some(scan => scan.status === 'COMPLETED') ? 'Есть выполненные проверки' : 'Ожидает первой проверки', enabled: scans.some(scan => scan.status === 'COMPLETED') }, { name: 'LLM Server', desc: 'llama.cpp + локальная модель', icon: BrainCircuit, state: 'Следующий этап', enabled: false }].map(service => <div className="service-card" key={service.name}><service.icon size={22} /><h3>{service.name}</h3><p>{service.desc}</p><span className={`tag ${service.enabled ? 'mint' : 'neutral'}`}>{service.state}</span></div>)}</div><p className="system-note">Worker размещается внутри Bult.ai. LLM Server будет подключён на следующем этапе, также на Bult.ai. Внешний LLM API не используется.</p></section>
          <div className="bottom-grid"><section className="panel"><div className="panel-heading"><div><h2>Модель данных</h2><p>9 таблиц приложения · Alembic migrations</p></div><Database size={19} /></div><div className="schema-list">{['users', 'projects', 'repositories', 'scans', 'findings', 'ai_analyses', 'fixes', 'rescans', 'scan_jobs'].map(table => <span key={table}><Layers size={13} />{table}</span>)}</div><div className="system-note">users пока содержит только владельца общего MVP workspace. Регистрация и вход не включены.</div></section><section className="panel"><div className="panel-heading"><div><h2>Состояние приложения</h2><p>Ответ настоящего backend</p></div><Activity size={18} /></div><dl className="system-facts"><div><dt>Версия</dt><dd>0.3.0</dd></div><div><dt>Database</dt><dd>{readiness?.database || 'unavailable'}</dd></div><div><dt>Schema revision</dt><dd>{readiness?.schema_revision || '—'}</dd></div><div><dt>Environment</dt><dd>{readiness?.environment || '—'}</dd></div></dl><a className="text-button" href="/api/openapi.json" target="_blank" rel="noreferrer">OpenAPI schema<ExternalLink size={13} /></a></section></div>
        </>}
        <footer className="footer"><span><ShieldCheck size={14} />BultShield AI <span className="footer-dot">·</span> Gitleaks v0.3.0</span><span>Detect <ArrowRight size={11} /> Understand <ArrowRight size={11} /> Fix <ArrowRight size={11} /> Verify</span></footer>
      </main>
    </div>

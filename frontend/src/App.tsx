...
function ProjectForm({ onClose, onSaved }: { onClose: () => void; onSaved: (project: Project) => void }) {
  ...
  return <Dialog title="Новый проект" onClose={busy ? () => {} : onClose}><form onSubmit={submit}>
    ...
    <p className="form-help">На этом этапе сохраняем настройки. Сканирование и доступность репозитория пока не проверяются.</p>
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    const results = await Promise.allSettled([api.readiness(), api.overview(), api.projects(), api.scans(), api.findings()] as const);
    setReadiness(results[0].status === 'fulfilled' ? results[0].value : null);
    if (results[1].status === 'fulfilled') setOverview(results[1].value); else setOverview(null);
    ...
    if (results.some(result => result.status === 'rejected')) setError('Не удалось обновить данные. Проверьте соединение приложения с PostgreSQL и повторите запрос.');
    setLoading(false);
  }, []);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="stage-progress"><span>Этап 02</span><span>Основа проекта</span></div><div className="stage-track"><span /></div><p>Следующий шаг — подключить сканеры.</p><div className="hosting-label"><Server size={15} /> Целевая платформа: Bult.ai</div></div>
    </aside>
    <div className="app-content">
      ...
      <main className="main-content">
        <div className="page-heading"><div><div className="eyebrow">BULTSHIELD AI / FOUNDATION</div><h1>{view === 'workspace' ? 'Безопасность начинается здесь.' : view === 'findings' ? 'Единый список находок' : view === 'activity' ? 'История сканирований' : 'Основа системы'}</h1><p>{view === 'workspace' ? 'Добавьте проект. Подготовьте репозиторий к первой проверке.' : view === 'findings' ? 'Результаты всех инструментов появятся в одном формате.' : view === 'activity' ? 'Каждый запуск, его статус и результат — в одном месте.' : 'Целевая платформа — Bult.ai. Архитектура готова к расширению.'}</p></div><div className="heading-actions"><button className="button secondary compact" onClick={() => void refresh()} disabled={loading} aria-label="Обновить данные"><RefreshCw size={16} className={loading ? 'spin' : ''} /><span>Обновить</span></button>{view === 'workspace' && <button className="button primary" onClick={() => setCreateOpen(true)} disabled={!isReady}><Plus size={16} />Новый проект</button>}</div></div>
        ...
        {view === 'workspace' && <>
          <section className="stats" aria-label="Состояние рабочего пространства">{[
            ...
            { title: 'Сканирования', value: overview?.scans, icon: ScanLine, foot: 'Сканеры ещё не подключены' },
            { title: 'Находки', value: overview?.findings, icon: ShieldEllipsis, foot: 'Появятся после первого scan' },
        ...
        {view === 'findings' && <section className="panel"><div className="panel-heading"><div><h2>Findings <span className="count-badge">{overview?.findings ?? '—'}</span></h2><p>Gitleaks · Semgrep CE · Trivy · Nuclei</p></div><label className="filter-label">Severity<select value={severity} onChange={event => setSeverity(event.target.value)}><option value="ALL">Все уровни</option>{['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO', 'UNKNOWN'].map(value => <option key={value}>{value}</option>)}</select></label></div>{filteredFindings.length ? <div className="result-list">{filteredFindings.map(finding => <article key={finding.id}><span className={`severity severity-${finding.severity.toLowerCase()}`}>{finding.severity}</span><div><h3>{finding.title}</h3><p>{finding.scanner} · {finding.file || finding.category}{finding.line_start ? `:${finding.line_start}` : ''}</p></div><span className="tag neutral">{finding.status}</span></article>)}</div> : <EmptyState icon={<ShieldEllipsis size={32} />} title={findings.length ? 'Нет находок с таким уровнем' : 'Проверок ещё не было'}>Отсутствие находок пока не означает отсутствие уязвимостей. Результаты появятся после подключения и запуска сканеров.</EmptyState>}</section>}
        {view === 'activity' && <section className="panel"><div className="panel-heading"><div><h2>Запуски <span className="count-badge">{overview?.scans ?? '—'}</span></h2><p>История сохраняется в PostgreSQL</p></div><span className="tag neutral">Scanners pending</span></div>{scans.length ? <div className="result-list">{scans.map(scan => <article key={scan.id}><ScanLine size={19} /><div><h3>{projects.find(project => project.id === scan.project_id)?.name || 'Project'}</h3><p>{date(scan.created_at)} · {scan.commit_sha?.slice(0, 7) || 'Нет commit SHA'}</p></div><span className="tag neutral">{scan.status}</span></article>)}</div> : <EmptyState icon={<Activity size={31} />} title="Первый scan ещё впереди">На Этапе 2 мы создаём основу проекта. Сканеры, очередь выполнения и повторные проверки будут подключены далее.</EmptyState>}<div className="lifecycle-strip">{['QUEUED', 'CLONING', 'SCANNING', 'NORMALIZING', 'AI_ANALYSIS', 'COMPLETED'].map((state, i) => <span key={state}>{i > 0 && <ArrowRight size={12} />}{state}</span>)}</div></section>}
        {view === 'system' && <>
          <section className="panel"><div className="panel-heading"><div><h2>Bult.ai infrastructure</h2><p>На Этапе 2 запускаются App и PostgreSQL</p></div><span className="tag mint">Этап 02</span></div><div className="service-grid">{[{ name: 'App', desc: 'React + FastAPI', icon: LayoutDashboard, state: isReady ? 'Работает' : 'Проверяется', enabled: isReady }, { name: 'PostgreSQL', desc: 'Данные + схема очереди', icon: Database, state: isReady ? 'Подключена' : 'Нет соединения', enabled: isReady }, { name: 'Worker', desc: 'Четыре scanner CLI', icon: ScanLine, state: 'Следующий этап', enabled: false }, { name: 'LLM Server', desc: 'llama.cpp + локальная модель', icon: BrainCircuit, state: 'Следующий этап', enabled: false }].map(service => <div className="service-card" key={service.name}><service.icon size={22} /><h3>{service.name}</h3><p>{service.desc}</p><span className={`tag ${service.enabled ? 'mint' : 'neutral'}`}>{service.state}</span></div>)}</div><p className="system-note">Worker и LLM Server также будут размещены внутри Bult.ai. Внешний LLM API не используется.</p></section>
          <div className="bottom-grid"><section className="panel"><div className="panel-heading"><div><h2>Модель данных</h2><p>9 таблиц приложения · Alembic migrations</p></div><Database size={19} /></div><div className="schema-list">{['users', 'projects', 'repositories', 'scans', 'findings', 'ai_analyses', 'fixes', 'rescans', 'scan_jobs'].map(table => <span key={table}><Layers size={13} />{table}</span>)}</div><div className="system-note">users пока содержит только владельца общего MVP workspace. Регистрация и вход не включены.</div></section><section className="panel"><div className="panel-heading"><div><h2>Состояние приложения</h2><p>Ответ настоящего backend</p></div><Activity size={18} /></div><dl className="system-facts"><div><dt>Версия</dt><dd>0.2.0</dd></div><div><dt>Database</dt><dd>{readiness?.database || 'unavailable'}</dd></div><div><dt>Schema revision</dt><dd>{readiness?.schema_revision || '—'}</dd></div><div><dt>Environment</dt><dd>{readiness?.environment || '—'}</dd></div></dl><a className="text-button" href="/api/openapi.json" target="_blank" rel="noreferrer">OpenAPI schema<ExternalLink size={13} /></a></section></div>
        </>}
        <footer className="footer"><span><ShieldCheck size={14} />BultShield AI <span className="footer-dot">·</span> Foundation v0.2.0</span><span>Detect <ArrowRight size={11} /> Understand <ArrowRight size={11} /> Fix <ArrowRight size={11} /> Verify</span></footer>
      </main>
    </div>
